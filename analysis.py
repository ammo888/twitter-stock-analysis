...
def perform_analysis(twitter_handle, stock_ticker, analysis_date, dummy_sentiments=False):
    ...
    tweet_count_plot = aggregated_sentiment_df.final_count.hvplot.line(title=f"Tweet counts for @{twitter_handle}")

    signal_corr = correlate(stock_df.Change, aggregated_sentiment_df.final_sentiment)
    ...
    plot = hv.Layout(stock_plot + stock_change_plot + sentiment_plot + tweet_count_plot + correlation_plot).cols(1)
